...
(function () {
  ...
  class SealionViewer {
    constructor(containerOrSelector, alignment, options) {
      ...
      try{
        ...
        if(!scroller){ scroller = document.createElement('div'); scroller.id = 'alignment-scroll'; alignment.appendChild(scroller); }
        ...
        const onKeyDown = (ke)=>{
          ...
          try{
            ...
            if(ke.key === 'ArrowLeft' || ke.key === 'ArrowRight' || ke.key === 'ArrowUp' || ke.key === 'ArrowDown'){
              ...
              // Meta (Command) pressed: page scroll with animation handled by the viewer
              if(isMeta){
                if(ke.key === 'ArrowLeft'){
  ...
  const endX = (colOffsets[visible.lastCol] || 0) - visible.scrollLeft + 3;
    ...
    drawAll() {
      // Clients will implement drawing routines during migration.
      // For now this is a safe no-op so adding the file doesn't change behavior.
      // e.g. this.drawSequences(); this.drawOverview();
    }
